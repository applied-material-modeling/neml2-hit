...
NB_MODULE(_nmhit, m)
{
  ...
  nb::register_exception_translator([](const std::exception_ptr & ep, void *) {
    ...
    catch (const nmhit::Error & e)
    {
      ...
      if (!msg_str)
        return; // MemoryError already set
  ...
  nb::class_<Node>(m, "Node")
    ...
    .def_prop_ro("filename", [](const Node & n) -> std::filesystem::path { return n.filename(); })
    ...
    .def(
      "param_optional_bool",
      [](const Node & n, const std::string & p, bool d) { return n.param_optional<bool>(p, d); },
      nb::arg("relpath"),
      nb::arg("default_val"))
    .def(
      "param_optional_int",
    ...
    .def(
      ...
      [](Node & parent, const std::string & relpath) -> std::unique_ptr<Node> {
        ...
        // find() may return a deep descendant; remove via its direct parent
        // so that nested relpaths (e.g. "a/b/k") work correctly.
        Node * actual_parent = child->parent();
        if (!actual_parent)
          throw nmhit::Error("remove_child: node at '" + relpath + "' has no parent");
        return actual_parent->remove_child(child);
      },
      nb::arg("relpath"))
    // render / clone
  ...
  nb::class_<Root, Node>(m, "Root").def(nb::init<>()).def("__repr__", [](const Root &) {
    return "<Root>";
  });
  ...
  nb::class_<Field, Node>(m, "Field")
    .def(
      nb::init<const std::string &, const std::string &>(), nb::arg("name"), nb::arg("raw_value"))
    .def("raw_val", &Field::raw_val)
    .def("set_val", &Field::set_val, nb::arg("value"))
    .def("__repr__",
         [](const Field & f) { return "<Field " + f.path() + "=" + f.raw_val() + ">"; });
  ...
  nb::class_<Blank, Node>(m, "Blank").def(nb::init<>()).def("__repr__", [](const Blank &) {
    return "<Blank>";
  });
